...
with ManagedScreen() as s:
    ...
    while not current_sand or current_sand[1] < lowest_y:
        if not current_sand:
            current_sand = 500, 0
        
        # move sand
        if get_block((current_sand[0], current_sand[1] + 1)) == AIR:
        ...
        # move camera down if sand reaches lower edge
        if current_sand:
            cam_pos = cam_pos[0], max(cam_pos[1], current_sand[1] - (cam_dims[1] * 3 // 4))
